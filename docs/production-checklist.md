...
Use this as a go-live gate for Agent Runtime deployments.
...
- [ ] `AGENT_RUNTIME_LLM_PROVIDER` (with matching `AGENT_RUNTIME_LLM_BASE_URL`, `AGENT_RUNTIME_LLM_MODEL`, and `AGENT_RUNTIME_LLM_API_KEY` when required) configured for your OpenAI-compatible provider, Claude, or local runtime.
- [ ] At least one channel token configured (`AGENT_RUNTIME_TELEGRAM_TOKEN` and/or `AGENT_RUNTIME_DISCORD_TOKEN`).
- [ ] `AGENT_RUNTIME_IMAP_*` and `AGENT_RUNTIME_SMTP_*` set only if email workflows are enabled.
- [ ] `AGENT_RUNTIME_SANDBOX_ALLOWED_COMMANDS` reduced to minimum required commands.

## B. Security Baseline

- [ ] Admin endpoint configured with mTLS (`ADMIN_HOST` + Caddy client cert trust).
- [ ] `AGENT_RUNTIME_ADMIN_TLS_SKIP_VERIFY=false` for operator clients.
...
- [ ] Verified ability to disable risky execution quickly:
  - [ ] `AGENT_RUNTIME_SANDBOX_ENABLED=false`
